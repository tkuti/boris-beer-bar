...
function renderHomePage() {
  ...
  createAnyElement('img', _homeWrapper, {
    src: './images/boris.png',
    alt: 'Boris'
  })
  createAnyElement('h1', _homeWrapper, {
    innerHTML: 'Hey Boris!',
    class: 'home-heading'
  })
  createAnyElement('p', _homeWrapper, {
    innerHTML: 'Welcome to your Beer App!'
  })
  createAnyElement('p', _homeWrapper, {
    innerHTML: 'Click the Lists button above and enjoy your work!'
  })
}
...
function renderListsPage() {
  ...
  createAnyElement('p', _listsWrapper, {
    innerHTML: 'Choose a list you wish to render'
  })
  const _listSelect = createAnyElement('select', _listsWrapper, {
    name: 'list-select',
    id: 'list-select'
  })
  createAnyElement('option', _listSelect, {
    value: 0,
    innerHTML: '--- select a list ---',
    disabled: true
  })
  ...
  createAnyElement('option', _listSelect, {
    value: 4,
    innerHTML: 'Without allergies'
  })
  createAnyElement('option', _listSelect, {
    value: 5,
    innerHTML: 'By water ratio'
  })
  createAnyElement('option', _listSelect, {
    value: 6,
    innerHTML: 'By rounded price'
  })
...
function renderInput(listType) {
  const _inputWrapper = createAnyElement('div', _listContainer, {
    class: 'input-wrapper'
  })
  const _typeSelect = createAnyElement('select', _inputWrapper)
  createAnyElement('option', _typeSelect, {
    value: 'none',
    innerHTML: '--- select an option ---',
    disabled: true
  })
  const options = listType === 'type' ? getBeerTypes() : getIngredients()
  ...
  _typeSelect.addEventListener('change', e => {
    listType === 'type'
      ? renderTable(
          filterByTypeApp(e.target.value),
          `Beers by type - ${e.target.value}`
        )
      : renderTable(
          getWithoutAllergiesApp(e.target.value),
          `Beers without allergies - ${e.target.value}`
        )
  })
}
...
function renderTable(list, title) {
  ...
  if (keys) {
    ...
    keys.forEach(key =>
      createAnyElement('th', _rowTh, { class: 'table-heading', innerHTML: key })
    )
    list.forEach((beer, index) => {
      const _row = createAnyElement('tr', _table)
      createAnyElement('td', _row, { innerHTML: index + 1, class: 'cell' })
      keys.forEach(key =>
        createAnyElement('td', _row, { class: 'cell', innerHTML: beer[key] })
      )
    })
  } else {
...
function getBeerTypes() {
  return [...new Set(beers.map(beer => beer.type))]
}

function getIngredients() {
  return beers[0].ingredients.map(ingredient => ingredient.name)
}
